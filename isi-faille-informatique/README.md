...
De la même manière, il est possible d'altérer une requete `INSERT` pour interférer dans la valeur des champs qui seront insérés en base.
...
Rappelez vous que vous pouvez utiliser ` -- ` pour ignorer le reste de la ligne (un espace doit être avant et après les deux tirets).


L'exploitation d'injections SQL n'est pas limitée à la modification de données. En supposant l'existence d'une autre table dans la base, imaginez un moyen d'utiliser cette faille d'injection SQL pour obtenir des informations sur les données de cette autre table (il n'est pas demandé de l'implémenter, mais d'expliquer une approche envisageable)
